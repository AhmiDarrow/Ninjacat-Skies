...
public enum GuardianKind {
    ...
    BEDDOWN("beddown", Strand.SOIL, "rootheart", 12.7F, 10.0F, 320, 12, 0x6B8E3A, Tier.GATE),
    GRINDMAW("grindmaw", Strand.STONE, "grindcore", 12.9F, 9.0F, 380, 14, 0x8A8580, Tier.GATE),
    THORNMOTHER("thornmother", Strand.SPROUT, "thornseed", 12.5F, 9.0F, 360, 10, 0x5AAF5A, Tier.GATE),
    EDGEWALKER("edgewalker", Strand.CLAW, "edgestep", 7.5F, 9.0F, 300, 16, 0x8C8C96, Tier.GATE),
    DRUMHEART("drumheart", Strand.SPARK, "drumpulse", 13.0F, 9.0F, 420, 15, 0xD4A84B, Tier.GATE),
    COGWRIGHT("cogwright", Strand.CLOCK, "cogloop", 9.5F, 12.0F, 440, 14, 0xC87A3A, Tier.GATE),
    HIVEMIND("hivemind", Strand.SWARM, "hivecall", 13.0F, 7.0F, 300, 10, 0xE6C478, Tier.GATE),
    SEALBREAKER("sealbreaker", Strand.SIGIL, "sealmark", 13.5F, 8.0F, 460, 16, 0x8A5FB8, Tier.GATE),
    UNWOVEN("unwoven", Strand.SPINDLE, "loomthread", 13.5F, 9.0F, 600, 18, 0x3D7A7A, Tier.GATE),
    // ---- the two easy
    LINTGOLEM("lintgolem", null, "lintwisp", 6.0F, 5.0F, 80, 3, 0xB8B4BC, Tier.EASY),
    TANGLE("tangle", null, "knotcharm", 7.0F, 10.0F, 160, 8, 0xC9A45C, Tier.EASY),
    // ---- the two insane (locked until after the Reweave)
    FIRSTCUT("firstcut", null, "firstcut_shard", 32.0F, 16.0F, 1800, 30, 0x2A1838, Tier.INSANE),
    OVERWEAVER("overweaver", null, "overweaver_shuttle", 40.0F, 22.0F, 2400, 26, 0x6C4FB0, Tier.INSANE);
    ...
    GuardianKind(String id, @Nullable Strand strand, String relicId, float height, float width, int baseHealth, int attackDamage, int colour, Tier tier) {
        this.id = id; this.strand = strand; this.relicId = relicId; this.height = height; this.width = width;
    ...
    public Component unlockName() {
        if (strand != null) return Component.translatable("message.guardians.totem.unlock_strand", strand.title());
        return switch (this) {
            case LINTGOLEM -> Component.translatable("message.guardians.totem.unlock_strand", Strand.SOIL.title());
            case TANGLE -> Component.translatable("message.guardians.totem.unlock_strand", Strand.CLAW.title());
            default -> Component.translatable("message.guardians.totem.unlock_reweave");
        };
    }
    /** The on-screen name ("the Beddown"), lowercase article as it reads mid-sentence; boss bar and messages. */
    public String titleKey() { return "message.guardians.title." + id; }
    public MutableComponent titleComponent() { return Component.translatable(titleKey()); }
