...
/**
 * CPU linear-blend skinning of a baked .ncgb mesh and the two-pass draw (lit albedo, then additive full-bright
 * emissive). Shared by the Guardians and any other renderer that draws the same mesh format. One instance per
 * renderer: it keeps scratch buffers between frames.
 */
public final class MeshPose {
    ...
    /** Interpolate clip frames f0 -> f1 by t into the bone matrices. */
    public void pose(GuardianModel.Clip clip, int f0, int f1, float t) {
        int nb = clip.bones;
        if (bones.length < nb*12) bones = new float[nb*12];
        for (int i = 0; i < nb*12; i++) bones[i] = Mth.lerp(t, clip.m[f0*nb*12 + i], clip.m[f1*nb*12 + i]);
    }

    public void draw(GuardianModel m, PoseStack.Pose pose, MultiBufferSource buf, ResourceLocation texture, ResourceLocation emissive, int light, int overlay) {
        for (GuardianModel.Part part : m.parts) {
            skin(part);
            ResourceLocation tex = part.textured ? texture : WHITE;
            ...
            if (part.textured) emit(buf.getBuffer(RenderType.eyes(emissive)), part, pose, 0xF000F0, OverlayTexture.NO_OVERLAY, false);
