...
public class CogwrightGuardian extends GuardianEntity {
    ...
    @Override protected MutableComponent immuneMessage() { return Component.translatable("message.guardians.cogwright.cogwright_s_chassis_shut_step"); }

    @Override
    protected void tickMechanic() {
        if (ageInFight == 1) { shout("message.guardians.cogwright.cogwright_answers_for_cut_winds"); stage = Stage.IDLE; stageTicks = -40; }
        Vec3 o = origin();
        stageTicks++;
        if (ageInFight % 200 == 0) { dir = -dir; sound(SoundEvents.NOTE_BLOCK_BELL.value(), 2F, 0.6F); say("message.guardians.cogwright.pendulum_swings_back_rings_reverse"); }
        ...
        switch (stage) {
            ...
            case OPEN -> { if (stageTicks >= OPEN_TICKS) { say("message.guardians.cogwright.chassis_snaps_shut"); setStage(Stage.IDLE); } }
        }
        if (tickCount % 40 == 0 && arena() != null && Mech.horiz(position(), o) > 31) getNavigation().moveTo(o.x, o.y, o.z, 1.0);
    }

    @Override protected void onPhase(int phase) { shout("message.guardians.cogwright.rewinds", 3 + phase); if (stage != Stage.OPEN) setStage(Stage.IDLE); }
    ...
    private void tickWind(Vec3 o) {
        ...
        if (stageTicks >= SHOW_GAP * sequence.size() + 10) { setStage(Stage.SOLVE); relight(o); shout("message.guardians.cogwright.now_walk_tiles_that_order"); }
    }
    ...
    private void tickSolve(Vec3 o) {
        ...
        for (ServerPlayer p : party()) {
            ...
            for (int i = progress + 1; i < sequence.size(); i++) if (on.equals(tile(o, sequence.get(i)[0], sequence.get(i)[1]))) { fail("message.guardians.cogwright.wrong_tile"); return; }
        }
        if (stageTicks >= SOLVE_TICKS) fail("message.guardians.cogwright.escapement_ran_out");
    ...
    private void succeed() {
        ...
        shout("message.guardians.cogwright.pattern_holds_cogwright_s_chassis"); sound(SoundEvents.IRON_DOOR_OPEN, 2F, 0.5F);
        Mech.burst(serverLevel(), ParticleTypes.ELECTRIC_SPARK, position().add(0, kind.height * 0.5, 0), 40, 4);
    }

    private void fail(String whyKey) {
        unlightAll(); Vec3 o = origin();
        for (int k = 0; k < TOWERS; k++) Mech.arc(serverLevel(), Mech.polar(o, TOWER_R * ringScale(), Math.PI / 3 * k, o.y));
        areaDamage(o, 40 * ringScale(), 6, 0.2); shout("message.guardians.cogwright.gear_towers_arc", Component.translatable(whyKey));
        setStage(Stage.IDLE);
    }
